...
export const registerUni = async (user, dispatch, navigate) => {
  dispatch(registerStart());
  try {
    await axios.post(`${baseURL}/auth/uni-register`, user);
    dispatch(registerSuccess());
    navigate("/login");
  } catch (err) {
    console.log(err.response.data);
...
export const updateUser = async (dispatch, user, id, token, token2) => {
  dispatch(updateStart());
  try {
...
export const followUser = async (
  ...
  try {
    ...
    const res = await axios.put(`${baseURL}/user/follow/${id}`, userId, {
      headers: { token: `Bearer ${token}` },
    });
...
export const upvotePost = async (dispatch, token, id, userId) => {
  ...
  try {
    await axios.put(`${baseURL}/post/like/${id}`, userId, {
      headers: { token: `Bearer ${token}` },
    });
